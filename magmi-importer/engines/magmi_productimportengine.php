...
class Magmi_ProductImportEngine extends Magmi_Engine
{
    ...
    public function engineRun($params, $forcebuiltin = array())
    {
        ...
        /*------- Mvaisberg Simples Creator based in configurable --------*/
        if(isset($params['preProcessCsv'])){
            $filename = basename($this->datasource->getCsvReader()->getFilename());
//            var_dump($this->datasource);die;
            $arrayData = $this->datasource->toArray();
            $this->addSimplesFromConfigurables($arrayData,$filename);
            $newfilename = '../../var/import/procesados/'.$filename;
            $this->datasource->getCsvReader()->setFilename($newfilename);
            $this->datasource = $this->getDataSource();
        }
        /*------- End mvaisberg --------*/
    ...
    //FUNCTIONS ABOUT CREATE SIMPLES UNDER CONFIGURABLES

    public function addSimplesFromConfigurables($data = array(),$filename){
        if(!empty($data)){

            $path = '../../var/import';
            $pathImages = '../../media/import';
            $header = array_keys($data[0]);


            array_push($header,'simples_skus');
            array_push($header,'configurable_attributes');
            $arrayToMerge = array();
            foreach ($data as &$line) {
                if ($line['type'] == "configurable") {
                    $conf_attrs = explode(',', "color,size");
                    $options = array();
                    foreach ($conf_attrs as $attr) {
                        $options[] = explode(',', ($line[$attr]));
                        $line[$attr] = '';
                    }
                    ...
                    foreach ($combinations as $combination) {
                        ...
                        if (is_array($combination)) {
                            foreach ($combination as $key => $option) {
                                $newline[$conf_attrs[$key]] = $option;
                            }
                        }else{
                            $newline[$attr] = $combination;
                        }
                        $newline['type'] = 'simple';
                        if(count($combination)>1) {
                            $sku = $newline['sku'] . '-' . implode('-', $combination);
                        }else{
                            $sku = $newline['sku'] . '-' . $combination;
                        }
                        array_push($simples_skus,$sku);
                        $sku = str_replace(' ', '%', $sku);
                        $newline['sku'] = $sku;
                        $newline['simples_skus'] = '';
                        $newline['configurable_attributes'] = '';
                        $newline['image'] = '';
                        $newline['media_gallery'] = '';
                        $arrayToMerge[] = $newline;
                    }
                    $line['simples_skus'] = implode(',',$simples_skus);
                    $line['configurable_attributes'] = 'color,size';
                }else{
                    $line['simples_skus'] = '';
                    $line['configurable_attributes'] = '';
                }
            }
            $newCsv = array_merge($data, $arrayToMerge);
            $this->imageAsociation($newCsv,$pathImages,$header); //PROCESADOR DE IMAGENES
            array_unshift($newCsv, $header);
            $newDir = $path.'/procesados/'.$filename;
            if(file_exists($newDir)) {
                unlink($newDir);
            }
            $this->generateCsv($newCsv, $filename);
        }
    }

    public function imageAsociation(&$data,$path,&$header){
        array_push($header,'image','media_gallery');
        $images = scandir($path);
        unset($images[0],$images[1]); //Saco ./ y ../
        foreach($data as &$line){
            $sku = $line['sku'];
            $i = 0;
            $media_gallery = array();
            foreach($images as $image){
                if($line['type'] == 'simple') {
                    if(isset($line['color']) && isset($line['size']) && $line['color'] != '' && $line['size'] != ''){
                        $skuSplit = explode('-',$sku);
                        $sku = $skuSplit[0].'-'.$skuSplit[1];
                    }
                    if (strpos($image, $sku) !== false) {
                        if ($i == 0) {
                            $line['image'] = '+/' . $image;
                        } else {
                            array_push($media_gallery, '/' . $image);
                        }
                        $i++;
                    }
                }else{
            ...
            if(!empty($media_gallery)){
                $line['media_gallery'] = implode(';',$media_gallery);
            }else{
                $line['media_gallery'] = '';
            }
        }
    ...
    public function generateCsv($data,$path) {
        $delimiter = $this->datasource->getCsvReader()->getCsep();
        $handle = fopen('/usr/share/nginx/www/var/import/procesados/'.$path, 'ab+');
        $last = count($data[0]);
